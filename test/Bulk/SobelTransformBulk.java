...
import transforms.SobelTransformation;
...
public class SobelTransformBulk {
	...
	public static void recurse(File f, String name) throws IOException,
		...
		if (f.isFile()) {
			...
			img.load(f);
			Transformations trans = new SobelTransformation(img);
			...
			int firstIndex = (name.lastIndexOf("Simulated_Data") + "Simulated_Data"
					.length());
			String nameFT = name.substring(0, firstIndex) + "_sobel"
